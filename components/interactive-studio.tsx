...
type PackageName = "Landing Page" | "Company Profile" | "Custom Website";
type PackageConfig = {
  base: number;
  max: number;
  includedUnits: number;
  unitLabel: "section" | "halaman";
  featureCount: number;
  revision: string;
...
const packages: Record<PackageName, PackageConfig> = {
  "Landing Page": { base: 400000, max: 600000, includedUnits: 5, unitLabel: "section", featureCount: 7, revision: "1× revisi minor", includedAddOns: [] },
  "Company Profile": { base: 900000, max: 1400000, includedUnits: 6, unitLabel: "halaman", featureCount: 12, revision: "2× revisi", includedAddOns: ["CMS", "Blog / Berita"] },
  "Custom Website": { base: 2000000, max: Infinity, includedUnits: 8, unitLabel: "halaman", featureCount: 15, revision: "3× revisi", includedAddOns: ["CMS", "Booking System", "Katalog Produk", "Admin Dashboard", "Integrasi API"] },
};
const addOns: Record<string, number> = { CMS: 300000, "Booking System": 500000, "Katalog Produk": 600000, "Admin Dashboard": 1000000, "SEO Lanjutan": 300000, "Blog / Berita": 400000, "Multi-language": 500000, "Integrasi API": 600000 };

export function PriceCalculator() {
  const [type, setType] = useState<PackageName>("Company Profile");
  const [pages, setPages] = useState(6);
  const [features, setFeatures] = useState<string[]>([]);
  const plan = packages[type];
  const availableAddOns = Object.keys(addOns).filter((feature) => !plan.includedAddOns.includes(feature));
  const pageCost = Math.max(0, pages - plan.includedUnits) * 80000;
  ...
  const scopeLabel = `${pages} ${plan.unitLabel}`;
  const query = new URLSearchParams({ type, pages: String(pages), features: [scopeLabel, ...features].join(", "), estimate: String(total) }).toString();
  function changeType(value: PackageName) { setType(value); setPages(packages[value].includedUnits); setFeatures([]); }
  ...
  return <div className="grid gap-8 lg:grid-cols-[1.1fr_.9fr]">
    <div><label className="label">Jenis website<select className="field" value={type} onChange={(event) => changeType(event.target.value as PackageName)}>{Object.keys(packages).map((item) => <option key={item}>{item}</option>)}</select></label><div className="mt-6 border border-black/15 bg-[var(--paper)] p-4 text-sm"><p className="m-0 font-bold">Sudah termasuk dalam paket</p><p className="mb-0 mt-2 text-[var(--muted)]">{plan.includedUnits} {plan.unitLabel} · {plan.featureCount} fitur · {plan.revision}</p></div><label className="label mt-6">Jumlah {plan.unitLabel}: <strong>{pages}</strong><input className="accent-[var(--accent)]" type="range" min="1" max="15" value={pages} onChange={(event) => setPages(Number(event.target.value))}/></label><fieldset className="mt-7"><legend className="mb-3 text-sm font-bold">Add-on di luar paket</legend><div className="flex flex-wrap gap-2">{availableAddOns.map((feature) => <button type="button" onClick={() => toggle(feature)} key={feature} aria-pressed={features.includes(feature)} className={`min-h-11 border px-3 text-sm font-semibold ${features.includes(feature) ? "border-[var(--ink)] bg-[var(--accent)]" : "border-black/20 bg-[var(--paper)]"}`}>{feature} · {rupiah(addOns[feature])}</button>)}</div></fieldset></div>
    <aside className="bg-[var(--ink)] p-7 text-[var(--paper)] md:p-9"><p className="text-sm text-white/60">Estimasi investasi</p><p className="mt-3 text-4xl font-black md:text-5xl">{rupiah(total)}</p><dl className="mt-7 grid gap-3 border-y border-white/20 py-5 text-sm"><div className="flex justify-between gap-5"><dt>Paket dasar</dt><dd>{rupiah(plan.base)}</dd></div><div className="flex justify-between gap-5"><dt>{plan.unitLabel === "section" ? "Section" : "Halaman"} tambahan</dt><dd>{rupiah(pageCost)}</dd></div><div className="flex justify-between gap-5"><dt>Add-on</dt><dd>{rupiah(featureCost)}</dd></div></dl>{exceeds && <p className="mt-5 border-l-2 border-[var(--accent)] pl-4 text-sm text-white/80">Total melewati range {type}. Scope akan ditinjau ke paket yang lebih sesuai agar breakdown dan pengerjaannya tetap jelas.</p>}<Link href={`/mulai-project?${query}`} className="button mt-8 w-full">Gunakan estimasi ini <ArrowRight size={17}/></Link><p className="mt-4 text-xs text-white/50">Estimasi awal. Quotation final dibuat setelah scope dikonfirmasi.</p></aside>
  </div>;
}
...
export function CalculatorSection() { return <section className="section" id="kalkulator"><div className="container"><p className="eyebrow">Price Calculator</p><h2 className="heading">Sudah tahu kebutuhannya? Hitung investasinya.</h2><p className="subheading mt-6">Lihat paket dasar, biaya section atau halaman tambahan, dan add-on di luar paket secara terpisah.</p><div className="mt-12 border border-black/20 bg-[var(--canvas)] p-5 md:p-9"><PriceCalculator/></div></div></section>; }
