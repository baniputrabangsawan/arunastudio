...
function scrollCurrentHomeToTop(event: MouseEvent<HTMLAnchorElement>) {
  if (
...
export function Header() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-black/15 bg-[var(--canvas)]/95 backdrop-blur-md">
    ...
    <div className="container flex h-[72px] items-center justify-between">
      <Link href="/" className="flex h-full items-center" aria-label="ARUNA, beranda" onClick={scrollCurrentHomeToTop}>
      ...
      <nav className="hidden items-center gap-7 md:flex" aria-label="Navigasi utama">
        {navItems.map((item) => <Link className="nav-link text-sm font-semibold text-[#4f5450]" key={item.href} href={item.href} onClick={item.href === "/" ? scrollCurrentHomeToTop : undefined}>{item.label}</Link>)}
      </nav>
    ...
    <div className={`absolute inset-x-0 top-full border-b border-black/10 bg-[var(--canvas)] px-5 transition-[opacity,transform] duration-200 md:hidden ${open ? "visible translate-y-0 opacity-100" : "invisible -translate-y-2 opacity-0"}`}>
      <nav className="container grid py-5" aria-label="Navigasi mobile">
        {navItems.map((item) => <Link className="border-b border-black/10 py-4 text-xl font-semibold" key={item.href} href={item.href} onClick={(event) => { setOpen(false); if (item.href === "/") scrollCurrentHomeToTop(event); }}>{item.label}</Link>)}
